...
if [ -r "$EDJARDIR/DSMP.jar" ] ; then
   ...
   if [ "x$Meeting" = x ] ; then
      ...
      echo "  Where Options:"
      echo "         -meeting    MeetingName  [Required                 ]"
      echo "         -srvmachine machine      [Defaults to local machine]"
      echo "         -srvport    port         [Defaults to 5000         ]"
      echo '         -user       username     [Defaults to $(whoami)    ]'
      exit 4
   fi 
  ...
  #
   ...
   echo "Starting MeetingCenter."
   echo "Connecting to $Machine:$Port for [$Meeting] as $User"
   exec $CMD $JAVAOPTS oem.edge.ed.odc.meeting.client.MessageCenter "$User" "$Meeting" "$Machine" "$Port"
else   
   echo "DSMP.jar not found. Cannot start MeetingCenter"
   exit 3
fi   
