...
if [ -d "$EDJARDIR/patches" ] ; then
   Patches=$(find "$EDJARDIR/patches" -name 'EDODCFTPClient*.jar' | sort -u -r)
...
if [ -r "$EDJARDIR/EDODCFTPClient.jar" ] ; then
  ...
  #
   ...
   export CLASSPATH="$PATCHJARS$EDJARDIR/EDODCFTPClient.jar:$EDJARDIR/swing.jar"
   
   export LIBPATH="$LIBPATH:$EDLIBDIR"
   export LD_LIBRARY_PATH="$LD_LIBRARY_PATH:$EDLIBDIR"
   export SHLIB_PATH="$SHLIB_PATH:$EDLIBDIR"
      
   echo "Starting FileTransfer client. Connecting to $1 port $port"
   $CMD $JAVAOPTS oem.edge.ed.odc.ftp.client.FileTransfer $1 $port
else   
   echo "EDODCFTPClient.jar not found. Cannot start FileTransfer"
   exit 3
fi   
