...
addEvent(window, "load", highlightTrashPkg);
...
function highlightTrashPkg()
{
    
     var str=normalizeString(document.WebDboxTrashForm.trashPkgToLit.value);
       
     if ( str != 'NoPackageSelected' )
     { 
        
	   //tweak ..for handling long package names..getElmntbyid complains..sigh !.. soo the highlight wudnt work !
	   //also provide a routine on serverside.. dboxpackageinfo.java	
       var newString=str.replace(new RegExp("\\s", "g"), "_"); 
		...
		rowhan.cells[6].className="rover"
...
function movePkgsToInbox()
{
 ...
 if (table == null )
 {
  ...
  for (i=0;i<cbxhan.length;i++)
      ...
      document.WebDboxTrashForm.packToTrash.value=filesIndices;
      document.WebDboxTrashForm.packTrashOper.value='true'
      document.WebDboxTrashForm.action='WebDboxTrashMovePkgToInbox.do';
      document.WebDboxTrashForm.submit();      
