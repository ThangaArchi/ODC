...
addEvent(window, "load", highlightDraftPkg);
...
function highlightDraftPkg()
	{
	    
	     var str=normalizeString(document.WebDboxDraftPkgForm.draftPkgToLit.value);	 	     
	     ...
	     { 
	       ...
	       var rowhan = document.getElementById(newString);
			...
			rowhan.cells[5].className="rover"				
	       document.WebDboxDraftPkgForm.litDraftPkgStatus.value=getLitPkgStatus(str); //used for sending a package	       
	     }
...
function getLitPkgStatus(pname)
{
    ...
    {
    document.WebDboxDraftPkgForm.action.value=WebDboxDraftPkgSend;
    document.WebDboxDraftPkgForm.submit()
    
    }
    
 
 
function deleteFilesFromSelDraft()
{
 
 
 var filesIndices='nofilestodelete';
 
 
  var cbxhan = document.getElementsByName('cbxHighLt');
  
  for (i=0;i<cbxhan.length;i++)
      {
      
       if (cbxhan[i].checked==true) //do whatever
  {
   if (filesIndices == 'nofilestodelete')
    filesIndices='filestodelete'
   filesIndices=filesIndices+','+i;   
   
  }
  
   }
      
      document.WebDboxDraftPkgForm.filesToDelete.value=filesIndices;
      document.WebDboxDraftPkgForm.fileDelOper.value='true'
      document.WebDboxDraftPkgForm.action='WebDboxDraftPkgDelFiles.do';
      document.WebDboxDraftPkgForm.submit();      
      
}

function delDraftPkgs()
{
 ...
 if (table == null )
 {
  ...
  for (i=0;i<cbxhan.length;i++)
      ...
      document.WebDboxDraftPkgForm.packToTrash.value=filesIndices;
      document.WebDboxDraftPkgForm.packTrashOper.value='true'
      document.WebDboxDraftPkgForm.action='WebDboxDraftPackagesDel.do';
      document.WebDboxDraftPkgForm.submit();      
