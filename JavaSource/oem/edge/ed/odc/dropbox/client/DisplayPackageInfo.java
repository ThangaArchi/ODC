...
public void copyPackageInfo(PackageInfo p) {
	...
	setPackagePoolId(p.getPackagePoolId());
	...
	if (p.getPackageDescription() != null && p.getPackageDescription().length() > 0) {
		try {
			...
			while (t.hasMoreTokens() && lcnt < 8) {
				String l = SearchEtc.htmlEscape(t.nextToken());
				int c = 0;
			...
			System.out.println("Error building description pop-up for package: " + getPackageName());
			System.out.println("Description is : " + getPackageDescription());
			e.printStackTrace();
