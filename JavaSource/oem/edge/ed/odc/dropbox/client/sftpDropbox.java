 ...
 **        (sftp/webdropbox)
 */
public class sftpDropbox implements SessionListener {
  ...
  // Allow 100k bytes of data to arrive before forcing wait by default
   protected int bufferedDownloadDataSize  =  100 * 1024;
   protected int bufferedUploadDataSize    =  100 * 1024;
   
  // Allow 2Meg bytes of data to be done in one shot
   protected int uploadTransferDataSize    =  2 * 1024 * 1024;
   protected int downloadTransferDataSize  =  2 * 1024 * 1024;
  ...
  /**
   * Set the maximum size of upload transfer
   *
   *@param v   Maximum transfer size to use when uploading a file. Default is 2MB
   */
   public void setMaxUploadTransferSize(int v)  { uploadTransferDataSize   = v; }
   
  /**
   * Set the maximum size of download transfer
   *
   *@param v   Maximum transfer size to use when downloading a file. Default is 2MB
   */
   public void setMaxDownloadTransferSize(int v)  { downloadTransferDataSize   = v; }
   
   public int  getMaxUploadTransferSize()    { return uploadTransferDataSize; }
   public int  getMaxDownloadTransferSize()  { return downloadTransferDataSize; }
  ...
  /*-=-=-=-=-=-=-=-=-=- Start Inner Classes -=-=-=-=-=-=-=-=-=-=-*/ 
   ...
   class MyClientDownloadOperation extends StreamingDownloadOperation 
      ...
      public MyClientDownloadOperation(DropboxAccess acc, ICAOutputStream oos, 
                                       long pid, long fid) {
         super(acc, oos, pid, fid);
         los = oos;
         los.getInputStream().setMaxBuffered(bufferedDownloadDataSize); 
         
         setMaxBufferSize(downloadTransferDataSize);
         
         setVerbose(DebugPrint.doDebug());
      }
   ...
   class MyPackageDownloadOperation extends PackageDownloadOperation 
      ...
      public MyPackageDownloadOperation(DropboxAccess acc, ICAOutputStream oos, 
                                        long pid, String encoding) {
         super(acc, oos, pid, encoding);
         los = oos;
         los.getInputStream().setMaxBuffered(bufferedDownloadDataSize); 
         setMaxBufferSize(downloadTransferDataSize);
         setVerbose(DebugPrint.doDebug());
   ...
   class MyClientUploadOperation extends StreamingUploadOperation
      ...
      public MyClientUploadOperation(DropboxAccess acc, ICAOutputStream oos, 
                                     long pid, long fid) {
         super(acc, oos.getInputStream(), pid, fid);
         los = oos;
         los.getInputStream().setMaxBuffered(bufferedUploadDataSize); 
         setMaxBufferSize(uploadTransferDataSize);
         setVerbose(DebugPrint.doDebug());
      }
   ...
   */
   public sftpDropbox(boolean dothatdebug) {
      ...
      if (dothatdebug) {
         ...
         DebugPrint.println(DebugPrint.DEBUG, "Debug is enabled");
      }
   }
  ...
  // For DropboxAccess health
   ...
   protected SessionHelper       sess_helper;
  ...
  // SessionListener method
   public void sessionUpdate(SessionEvent e) {
      if        (e.isShutdown()) {
         disconnect();
      } else if (e.isRefreshError()) {
         System.out.println("Error while refreshing sftpDropbox session for " +
            userV + "/" + companyV);
         SearchEtc.printStackTrace(e.getCause(), System.out);
      }
   }
...
//            facClassS = "oem.edge.ed.odc.dropbox.service.helper.JAXRPCConnectFactory";
   ...
   public void disconnect() {
      ...
      if (isConnectedV) {
         ...
         try {
           ...
           // We will get another call to our listener, but isConnectedV will
           //  be set to false, so won't repreocess. Check if we have a sess_helper. 
           //  We could have Connected, but failed to login ... which is where
           //  sess_helper is created
            if (sess_helper != null) {
               sess_helper.cleanup();
            }
   ...
   */
   public boolean login(String token) throws Exception {
     ...
     // Get session refreshing for free ... and autoclose if 1hr of inactivity
      ...
      sess_helper.addSessionListener(this);
     ...
     //setSessionId();
      ...
      sess_helper.setSessionInformation(clienttype);
   ...
   *
   *@param packname unique package name for package create
   *@return Returns true if operation worked, but not used, as if 
   *         unsuccessful, an exception is thrown
   */
   ...
   * Creates a new package with the specified name <i>packname</i> in the Drafts
   * folder. Package names MUST be unique in Drafts, so a failure will occur if
   * a uncommitted package with the same name already exists. The <i>poolid</i>
   * specifies which storage pool should be used for all files uploaded to the
   * package. 
   *
   *@param packname unique package name for package create
   *@param poolid storage pool id to be used when uploading files
   *@param msk    package flags mask   to augment
   *@param vals   package flags values to augment
   *@return Returns true if operation worked, but not used, as if 
   *         unsuccessful, an exception is thrown
   */
   public  boolean createPackage(String packname, 
                                 long poolid,
                                 int msk, int vals) throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:createPackage -Not connected!");
         throw new Exception("No Connected");
      }
      ...
      dropbox.createPackage(packname, null, poolid, msk, vals);
   ...
   *                       the public storage pool.
    *@param msk    Mask of boolean options being set for new package
    *@param vals Mask of boolean values     to set for new package
    *@return long          Newly created package id
    *        
    */
   public  long createPackage(String packname, String desc,  long poolid, 
        int msk, int vals) throws Exception {
       ...
       pkgid=dropbox.createPackage(packname, desc, poolid, msk, vals);
   ...
   *
   */
   public PoolInfo getStoragePoolInstance(long poolid) throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:getStoragePoolInstance - Not connected!");
         throw new Exception("No Connected");
      }
      if (!isLoggedInV) {
         System.out.println("sftpDropbox:getStoragePoolInstance - Not LoggedIn");
         throw new Exception("Not Logged in");
      }
   ...
   *
   */
   public Vector queryStoragePoolInformation() throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:queryStoragePoolInfo -Not connected!");
         throw new Exception("No Connected");
      }
      if (!isLoggedInV) {
         System.out.println("sftpDropbox:queryStoragePoolInfo - Not LoggedIn");
         throw new Exception("Not Logged in");
      }
   ...
   *
   */
   public Vector queryPackageAclCompanies(long packid) throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:queryPkgAclCompany - Not connected!");
         throw new Exception("No Connected");
      }
      if (!isLoggedInV) {
         System.out.println("sftpDropbox:queryPkgAclCompany - Not LoggedIn");
         throw new Exception("Not Logged in");
      }
      
      return dropbox.queryPackageAclCompanies(packid);
   }
   ...
   * @return Vector    List of company name strings
   * @throws     Exception describes failure of routine
   *
   */
   public Vector queryRepresentedCompanies(Vector acls, boolean needitar) throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:queryRepCompany - Not connected!");
         throw new Exception("No Connected");
      }
      if (!isLoggedInV) {
         System.out.println("sftpDropbox:queryRepCompany - Not LoggedIn");
         throw new Exception("Not Logged in");
      }
      
      return dropbox.queryRepresentedCompanies(acls, needitar);
   }
   ...
   *
   */
   public Vector lookupUser(String username, boolean isregex) throws Exception {
      if (!isConnectedV) {
         System.out.println("sftpDropbox:lookupUser - Not connected!");
         throw new Exception("No Connected");
      }
      if (!isLoggedInV) {
         System.out.println("sftpDropbox:lookupUser - Not LoggedIn");
         throw new Exception("Not Logged in");
      }
      
      return dropbox.lookupUser(username, isregex);
   }
   ...
   */
   ...
   public  Operation uploadFile(long packid, String file) throws Exception {
      return uploadFile(packid, file, StreamingUploadOperation.DEFAULT_SIZE);
   }
   
   public  Operation uploadFile(long packid, String file, long sz) throws Exception {
      if (!isConnectedV) {
      ...
      long itemid = dropbox.uploadFileToPackage(packid, file, sz);
      
      ICAOutputStream os = new ICAOutputStream(new ICAInputStream());
      
      MyClientUploadOperation cujo = new MyClientUploadOperation(dropbox, os, 
                                                                 packid, itemid);
      cujo.setTotalSize(sz);
      cujo.process();
   ...
   public  Operation downloadFile(long packid, long fileid) throws Exception {
      ...
      MyClientDownloadOperation cdo = new MyClientDownloadOperation(dropbox, os, 
                                                                    packid, fileid);
      cdo.process();
   ...
   */
   public  Operation downloadPackage(long packid, String encoding)
      ...
      MyPackageDownloadOperation cdo = 
         new MyPackageDownloadOperation(dropbox, os, packid, encoding);
      cdo.process();
