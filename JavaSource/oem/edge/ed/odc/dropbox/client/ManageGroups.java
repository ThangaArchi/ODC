...
public class ManageGroups extends JDialog {
	...
	private JButton editorDelBtn = null;
	...
	private JButton userDelBtn = null;
...
private class DeleteGroupListener implements ActionListener, Runnable {
	...
	public void run() {
		try {
			...
			errorHandler.addMsg(e.getMessage(),"Delete Group Failed");
		} catch(RemoteException re) {
			errorHandler.addMsg(re.getMessage(),"Delete Group Failed");
		}
...
private class EditHandler extends MouseAdapter implements ActionListener, ItemListener, ListSelectionListener, DocumentListener {
	public void actionPerformed(ActionEvent e) {
		try {
			...
			else if (e.getSource() == getEditorDelBtn()) {
				deleteBuddy(getEditorFromLB());
			}
	...
	public void valueChanged(ListSelectionEvent e) {
		if (e.getSource() == getUserToLB()) {
			...
			boolean enabled = getUserFromLB().getSelectedIndex() != -1;
			getUserAddBtn().setEnabled(enabled);
			getUserDelBtn().setEnabled(enabled);
		}
		else if (e.getSource() == getEditorToLB()) {
			getEditorRemBtn().setEnabled(getEditorToLB().getSelectedIndex() != -1);
		}
		else if (e.getSource() == getEditorFromLB()) {
			boolean enabled = getEditorFromLB().getSelectedIndex() != -1;
			getEditorAddBtn().setEnabled(enabled);
			getEditorDelBtn().setEnabled(enabled);
		}
	}
	...
	public void addNewEditor() {
		...
		while (list.hasMoreTokens()) {
			...
			// If not already a known buddy, add it to buddy and user list.
			if (! users.contains(editor)) {
				users.addElement(editor);
	...
	public void addNewUser() {
		...
		while (list.hasMoreTokens()) {
			...
			// If not already a known buddy, add it to buddy and from editor list.
	...
	public void deleteBuddy(JList lb) {
		Object[] objs = lb.getSelectedValues();
		boolean saveBuddyList = false;
		
		// Remove deleted buddies from both editor and users from list as
		// well as saved buddies list (if there).
		for (int i = 0; i < objs.length; i++) {
			getUserFromLM().removeElement(objs[i]);
			getEditorFromLM().removeElement(objs[i]);
			if (users.removeElement(objs[i])) {
				saveBuddyList = true;
			}
		}

		// Need to save buddy list?
		if (saveBuddyList) {
			users = buddyMgr.setBuddyList(users);
		}
		
		// Doesn't affect group definition, so no need to call adjustGUI.
	}
...
private class GroupSelectionHandler implements Runnable {
	...
	public void reset() {
		...
		getEditorTF().setText("");
		getEditorNewAddBtn().setEnabled(false);
		getEditorDelBtn().setEnabled(false);
	...
	public void run() {
		try {
			try {
				...
				errorHandler.addMsg(e.getMessage(),"Query Group Failed");
...
private class GroupActionListener extends GroupSelectionHandler implements ActionListener, Runnable {
	...
	public void save() {
		...
		// For new groups, make sure the name is lower case only.
		if (newGroup) {
			getNameTF().setText(getNameTF().getText().toLowerCase());
		}
			...
			if (savingGroup) {
				...
				if (newGroup) {
					try {
						...
						errorHandler.addMsg(e.getMessage(),"Create Group Failed");
						SwingUtilities.invokeLater(busyCursorOff);
						return;
					}
				}
				...
				// See if the attributes changed.
					...
					try {
						...
						errorHandler.addMsg(e.getMessage(),"Edit Attributes Failed");
						showEndDlg = false;
					}
				}
				...
				while (users.hasMoreElements()) {
					...
					try {
						...
						// Failed to add a user, remove from buddy list.
						if (e.getMessage() != null) {
							if (ManageGroups.this.users.removeElement(user)) {
								updatedUserList = true;
							}
						}

						errorHandler.addMsg(e.getMessage(),"Add Member Failed");
						showEndDlg = false;
					}
				}
				...
				while (users.hasMoreElements()) {
					...
					try {
						...
						errorHandler.addMsg(e.getMessage(),"Remove Member Failed");
						showEndDlg = false;
					}
				}
				...
				while (users.hasMoreElements()) {
					...
					try {
						...
						errorHandler.addMsg(e.getMessage(),"Add Editor Failed");
						showEndDlg = false;
					}
				}
				...
				while (users.hasMoreElements()) {
					...
					try {
						...
						errorHandler.addMsg(e.getMessage(),"Remove Editor Failed");
						showEndDlg = false;
					}
				}

				// Failed adds caused user deletes? Save buddy list.
				if (updatedUserList) {
					ManageGroups.this.users = buddyMgr.setBuddyList(ManageGroups.this.users);
				}
...
private javax.swing.JButton getEditorAddBtn() {
	if (ivjEditorAddBtn == null) {
		try {
			...
			ivjEditorAddBtn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/oem/edge/ed/odc/dsmp/client/upload.gif")));
...
private javax.swing.JPanel getEditorBtnPnl() {
	if (ivjEditorBtnPnl == null) {
		try {
			...
			constraintsEditorAddBtn.insets = new java.awt.Insets(0,0,0,0);
			...
			ivjEditorBtnPnl.add(getEditorAddBtn(), constraintsEditorAddBtn);
			getEditorBtnPnl().add(getEditorRemBtn(), constraintsEditorRemBtn);
			ivjEditorBtnPnl.add(getEditorDelBtn(), gridBagConstraints2);
		} catch (java.lang.Throwable ivjExc) {
			handleException(ivjExc);
		}
	}
...
private javax.swing.JButton getEditorNewAddBtn() {
	if (ivjEditorNewAddBtn == null) {
		try {
			...
			ivjEditorNewAddBtn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/oem/edge/ed/odc/dsmp/client/upload.gif")));
...
private javax.swing.JButton getEditorRemBtn() {
	if (ivjEditorRemBtn == null) {
		try {
			...
			ivjEditorRemBtn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/oem/edge/ed/odc/dsmp/client/download.gif")));
...
private javax.swing.JButton getUserAddBtn() {
	if (ivjUserAddBtn == null) {
		try {
			...
			ivjUserAddBtn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/oem/edge/ed/odc/dsmp/client/upload.gif")));
...
private javax.swing.JPanel getUserBtnPnl() {
	if (ivjUserBtnPnl == null) {
		try {
			...
			constraintsUserAddBtn.insets = new java.awt.Insets(0,0,0,0);
			...
			ivjUserBtnPnl.add(getUserAddBtn(), constraintsUserAddBtn);
			getUserBtnPnl().add(getUserRemBtn(), constraintsUserRemBtn);
			ivjUserBtnPnl.add(getUserDelBtn(), gridBagConstraints1);
		} catch (java.lang.Throwable ivjExc) {
			handleException(ivjExc);
		}
	}
...
constraintsUserToSP.gridheight = 0;
			...
			ivjUserPnl.add(getUserBtnPnl(), constraintsUserBtnPnl);
		} catch (java.lang.Throwable ivjExc) {
			handleException(ivjExc);
		}
	}
...
private javax.swing.JButton getUserRemBtn() {
	if (ivjUserRemBtn == null) {
		try {
			...
			ivjUserRemBtn.setIcon(new javax.swing.ImageIcon(getClass().getResource("/oem/edge/ed/odc/dsmp/client/download.gif")));
...
public void setDropboxAccess(DropboxAccess newDboxAccess) {
	...
	/**
	 * This method initializes userDelBtn	
	 * 	
	 * @return javax.swing.JButton	
	 */    
	private JButton getUserDelBtn() {
		if (userDelBtn == null) {
			userDelBtn = new JButton();
			userDelBtn.setIcon(new ImageIcon(getClass().getResource("/oem/edge/ed/odc/dropbox/client/logoff.gif")));
			userDelBtn.setMargin(new java.awt.Insets(0,0,0,0));
			userDelBtn.setToolTipText("Delete user from saved list");
			userDelBtn.addActionListener(editHandler);
		}
		return userDelBtn;
	}
	/**
	 * This method initializes editorDelBtn	
	 * 	
	 * @return javax.swing.JButton	
	 */    
	private JButton getEditorDelBtn() {
		if (editorDelBtn == null) {
			editorDelBtn = new JButton();
			editorDelBtn.setIcon(new ImageIcon(getClass().getResource("/oem/edge/ed/odc/dropbox/client/logoff.gif")));
			editorDelBtn.setMargin(new java.awt.Insets(0,0,0,0));
			editorDelBtn.setToolTipText("Delete user from saved list");
			editorDelBtn.addActionListener(editHandler);
		}
		return editorDelBtn;
	}
