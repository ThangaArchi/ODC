...
public class DropboxCmdline implements Runnable, TimeoutListener {
  ...
  // Sort Vector of Strings or Pathinfo objects
   ...
   DropboxAccess dropboxorig = null;
  ...
  // We process the Timeout here for REFRESHSESSIONID
   ...
   public void login(String token) throws Exception {
      ...
      h.put(DropboxAccess.ClientType, "dropboxftp");
  ...
  ** Cmdline specific entrypoints
  \* -------------------------------------------------------*/
   ...
   public long createPackage(String packname) throws Exception {
      ...
      return dropbox.createPackage(packname, null, storagepool, 0, 0);
   }
   ...
   public  Operation uploadFile(long packid, 
      ...
      UploadOperation cujo = new UploadOperation(dropbox, factory, f, packid, fileid);
   ...
   public  Operation downloadFile(File f,
      ...
      Operation cujo = new DownloadOperation(dropbox, factory, f, packid, fi);
      cujo.setNumberOfWorkers(numWorkers);
   ...
   public  Operation downloadPackage(File f, long packid, 
      ...
      PackageDownloadOperation cdo = 
         new PackageDownloadOperation(dropbox, factory, ostream, 
                                      packid, encoding);
   ...
   public Map listOptions(Vector v) throws Exception {
     ...
     // Add in storage pool session option
      ...
      if (v == null || v.contains("storagepool")) {
         opts.put("storagepool", ""+storagepool);
      }
   ...
   public PoolInfo getStoragePoolInstance(String p) throws Exception {
      ...
      while(it.hasNext()) {
         ...
         if ((""+pi.getPoolId()).equals(p) || (n != null && n.equals(p))) {
            return pi;
         }
      }
      throw new DboxException("Storage Pool with id/name of " + p + " not found");
   }
 ...
 */
   ...
   final static int CMD_LPWD        = 22;
   final static int CMD_LRMDIR      = 23;
   final static int CMD_LS          = 24;
   final static int CMD_LSP         = 25;
   final static int CMD_LSPKGDESC   = 26;
   final static int CMD_MARK        = 27;
   final static int CMD_MGET        = 28;
   final static int CMD_MGETPACK    = 29;
   final static int CMD_MKDIR       = 30;
   final static int CMD_MPUT        = 31;
   final static int CMD_PROMPT      = 32;
   final static int CMD_PUT         = 33;
   final static int CMD_PWD         = 34;
   final static int CMD_QUIT        = 35;
   final static int CMD_RESTART     = 36;
   final static int CMD_RM          = 37;
   final static int CMD_RMACL       = 38;
   final static int CMD_RMDIR       = 39;
   final static int CMD_RMGRP       = 40;
   final static int CMD_SETOPT      = 41;
   final static int CMD_SETPKGDESC  = 42;
   final static int CMD_SETPKGDESCF = 43;
   final static int CMD_SETPKGOPT   = 44;
   final static int CMD_TUNNEL      = 45;
   final static int CMD_UNMARK      = 46;
   final static int CMD_VERBOSE     = 47;
   final static int CMD_PARSEABLE   = 48;
   ...
   String commands_str[] = {
     // commands from sftp 
      ...
      "listacls", "[remote package]         - list the acls for the package",
...
// MPZ replace version checking...
     ...
     // If we have options
      try {
         ...
         if (printexception) ee.printStackTrace(System.out);
...
//                                  DateFormat.SHORT);
  ...
  // Should be static in PathInfo
   ...
   public File makelocalfile(String s) {
  ...
  // Return vector of String filenames
   public Vector fileWild(String wild, Vector ret) {
      ...
      File f = makelocalfile(wild);
      
     // Return of wild is null or null len
      if (wild == null || wild.length() == 0) return ret;
      
      StringTokenizer stok = new StringTokenizer(f.getAbsolutePath(), 
     ...
     // This should ALWAYS be true
      if (stok.hasMoreTokens()) candidates.add(File.separator);
         ...
         while(it.hasNext()) {
           ...
           // If this is the last token and we are stripping, just take the match and go
            ...
            try {
                     ...
                     if (true) {
                       ...
                       // Splatting package
                        ...
                        if (pinfo.file == null) {
                          ...
                          // If we make it here and we are stripping, just take 
                          //  the path as the rest of the tokens are files. 
                          //  Note, we add directly to ret so its not processed 
                          //  anymore
                           if (strip) {
               ...
               if (printexception) e.printStackTrace(System.out);
...
//               showmsg(101, true, pinfo.toString());
      ...
      while(pathenum.hasMoreElements()) {
        ...
        // Valid to add/remove ACL
         ...
         try {
            ...
            if (printexception) dbe.printStackTrace(System.out);
            if (addRemove) {
               showerror(102, true, 
  ...
  // Specific commands from user
   ...
   void cmd_setopt(Command c, String arr[], int numused) {
      ...
      try {
        // Treal storagepool option special
...
//      }
            ...
            while(todoenum.hasMoreElements()) {
               ...
               try {
                 ...
                 // If prompting
                  ...
                  if (printexception) ioe.printStackTrace(System.out);
   ...
   void cmd_verbose(Command c, String arr[], int numused) {
     ...
     //setDebug(false);
      ...
      if (vlev > 2) {
         ProxyDebug dbg = new ProxyDebug(dropboxorig);
         dbg.enableDebug();
         dropbox = (DropboxAccess)dbg.makeProxy();
      }
   }
