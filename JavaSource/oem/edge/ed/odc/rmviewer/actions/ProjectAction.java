...
public class ProjectAction extends Action {
	...
	public ActionForward execute(
		...
		try {
					...
					if("showProject".equals(mapping.getParameter()) )
					{
						...
						try{
							String serverDet[] = projectForm.getServerInstanceId().split("@"); 
							projectForm.setProjectName( serverDet[0] );
							projectForm.setServerName( serverDet[1] );
							projectForm.setGridPath( serverDet[2] );
			...
			if("createProject".equals(mapping.getParameter()) )
			{
				...
				if( (projectForm.getProjectName() != null) && (projectForm.getProjectName().length() > 0) ){
					try{
							String serverDet[] = projectForm.getServerInstanceId().split("@"); 
							projectForm.setServerInstanceId( serverDet[0] );
							projectForm.setServerName( serverDet[1] );
							projectForm.setGridPath( serverDet[2] );
					}catch(Exception ex) {
						logger.error(ex);
	...
	public ActionForward deleteProject(ActionMapping mapping, ProjectForm objProjectForm, HttpServletRequest request) throws Exception
	{
		...
		try{
			String serverDet[] = objProjectForm.getProjectId().split("@"); 
			objProjectForm.setProjectId( serverDet[0] );
			objProjectForm.setProjectName( serverDet[1] );
			objProjectForm.setServerName( serverDet[2] );
			objProjectForm.setGridPath( serverDet[3] );
		}catch(Exception ex){
			logger.error(ex);
			ex.printStackTrace();
		}
