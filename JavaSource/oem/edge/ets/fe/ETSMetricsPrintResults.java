...
public class ETSMetricsPrintResults  {

	static public StringBuffer printColumns(ETSMetricsResultObj resobj){
		StringBuffer buf = new StringBuffer();
		Vector cols = resobj.getColumnsToShow();
		
		buf.append("<tr><td colspan=\"2\" class=\"tblue\" height=\"18\">&nbsp;Select column(s) to display</td></tr>");
		if(resobj.getErrorFlag() && resobj.getErrorField().equals("columnSelOptions")){
			...
			buf.append("<tr><td colspan=\"2\"><a name=\"res\" id=\"res\" href=\"#res\"></a><span style=\"color:#ff3333;\"><b>Input error: \""+resobj.getErrorMsg()+"\"</b></span></td></tr>");
			buf.append("<tr><td colspan=\"2\"><img src=\"" + Defines.TOP_IMAGE_ROOT + "c.gif\" width=\"1\" height=\"5\" alt=\"\" /></td></tr>");
		}
		buf.append("<tr><td colspan=\"2\"><table border=\"0\" width=\"600\">");
		int iCol = 1;
	...
	static public StringBuffer printSearchResults(ETSMetricsResultObj resobj, boolean download){
		StringBuffer buf = new StringBuffer();
	...
	static public StringBuffer printResults(ETSMetricsResultObj obj){
		StringBuffer buf = new StringBuffer();
		...
		if(obj.getSearchResult()!=null){
			...
			if (v.size()>0){
				buf.append("<tr><td colspan=\""+cols.size()+"\" class=\"tblue\" height=\"18\">&nbsp;Report result(s)</td></tr>");
				...
				for (int icols = 0; icols<allcols.size();icols++){
					...
					if (cols.contains(ss[0])){
						buf.append("<th width=\""+ss[4]+"\" valign=\"top\">"+ss[2]+"</th>");
					}
				}	
				...
				for(int i=0;i<v.size();i++){
					...
					ETSMetricsObj m = (ETSMetricsObj)v.elementAt(i);
				...
				buf.append("<tr><td class=\"tblue\" width=\"600\" height=\"18\">&nbsp;Report result(s)</td></tr>");
	...
	static public StringBuffer getCSVResults(ETSMetricsResultObj obj, StringBuffer b){
		StringBuffer buf = new StringBuffer();
