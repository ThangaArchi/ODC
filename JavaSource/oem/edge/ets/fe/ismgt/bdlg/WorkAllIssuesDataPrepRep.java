...
/**
 * @author v2phani
 * This bean makes all the data reqd, for the Work for all existing issues report
 */
public class WorkAllIssuesDataPrepRep extends FilterDetailsDataPrepAbsBean {

	public static final String VERSION = "1.48";

	/**
	 * Constructor for WorkAllIssuesDataPrepRep.
	 * @param request
	 * @param response
	 * @param issobjkey
	 */
	public WorkAllIssuesDataPrepRep(HttpServletRequest request, HttpServletResponse response, EtsIssFilterObjectKey issobjkey) {
	...
	public EtsIssFilterDetailsBean getFilterDetails() throws SQLException, Exception {
		...
		//set the srch by num
		etsFilterBean.setSearchByNum(AmtCommonUtils.getTrimStr(getRequest().getParameter("isssrchnum")));

		if (isDefaultUsrSaveQryExists("WRKALL")) { //if user save qry exists
		...
		if (getIssobjkey().getIssueSubType().equals(ETSCHANGESUBTYPE)) {
			...
			//repTabList = getFilterDAO().getReportTabList(qryPrepBean);
		}
	...
	public void setDefaultFilterParams() {
		...
		if (getIssobjkey().getIssueSubType().equals(ETSCHANGESUBTYPE)) {
			...
			//			New to Submit
			//			Submitted > Assigned
			//			getDefStatusTypeList().add("Submit");
	...
	public void setFilterParams() throws SQLException, Exception {
		...
		if (!isDefaultUsrSaveQryExists("WRKALL") || isIssueIdSrchOn()) { //if user save does not exists
		...
		} else { //if user do have some default params

			setUserSaveSearchParams("WRKALL");
		}
	...
	public boolean isIssueIdSrchOn() {
		
		String srchNum= AmtCommonUtils.getTrimStr(getRequest().getParameter("isssrchnum"));
		
		if(AmtCommonUtils.isResourceDefined(srchNum)) {
