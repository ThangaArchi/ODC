...
import oem.edge.ets.fe.ismgt.bdlg.EtsIssSubscribeIssTypBdlg;
 ...
 */
public class EtsIssSubscribeIssTypeAction extends Action implements EtsIssFilterConstants, EtsIssueActionConstants {

	/**
	 * 
	 */
	public EtsIssSubscribeIssTypeAction() {
		super();
	...
	public int getSubsIssTypeSubActionState(EtsIssObjectKey etsIssObjKey) {
		...
		if (AmtCommonUtils.isResourceDefined(op)) {
			...
			if (op.equals("1200")) {
				...
				state = SUBSISSTYP1STPAGE;

			}

		}

		String op_1201 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_1201.x"));

		if (AmtCommonUtils.isResourceDefined(op_1201)) {

			state = SUBSISSTYPE;

		}

		String op_1202 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_1202.x"));

		if (AmtCommonUtils.isResourceDefined(op_1202)) {
	...
	public EtsIssTypeInfoModel getIssueTypeInfoDets(EtsIssObjectKey etsIssObjKey) throws SQLException, Exception {
		...
		int state = getSubsIssTypeSubActionState(etsIssObjKey);

		Global.println("current state in getIssueTypeInfoDets()===" + state);

		EtsIssSubscribeIssTypBdlg subsIssTypDataPrep = new EtsIssSubscribeIssTypBdlg(etsIssObjKey, state);

		switch (state) {

			case SUBSISSTYP1STPAGE :

				issTypeModel = subsIssTypDataPrep.getFirstPageDets();

				break;

			case SUBSISSTYPE :

				issTypeModel = subsIssTypDataPrep.subscribeIssueType();

				break;

			case UNSUBSISSTYPE :

				issTypeModel = subsIssTypDataPrep.unSubscribeIssueType();
	...
	public int processRequest(EtsIssObjectKey etsIssObjKey) {
		...
		try {
			...
			curstate = getSubsIssTypeSubActionState(etsIssObjKey);
	...
	String getForwardMap(int processrequest) {
		...
		switch (processrequest) {
			...
			case UNSUBSISSTYPE :
				...
				forward = "subsIssueType1stPage";

				break;

			case ERRINACTION :

				forward = "subsIssueTypeFail";
