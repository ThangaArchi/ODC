 ...
 */
public class EtsAddCustFieldAction extends Action implements EtsIssFilterConstants, EtsIssueActionConstants {

	/**
	 * 
	 */
	public EtsAddCustFieldAction() {
		super();
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (es.GetProfile(response, request)) {
				...
				String currentCustFieldId = AmtCommonUtils.getTrimStr(request.getParameter("hidCustFieldId"));				

				if(AmtCommonUtils.isResourceDefined(currentCustFieldId) )
					request.setAttribute("currentCustFieldId", currentCustFieldId);
	 ...
	 */
	public int getAddCustFieldSubActionState(EtsIssObjectKey etsIssObjKey) {
		...
		String op_2411 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_2411.x"));

		if (AmtCommonUtils.isResourceDefined(op_2411)) {

			state = ADDCUSTFIELDCONTINUE;

		}

		String op_2412 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_2412.x"));

		if (AmtCommonUtils.isResourceDefined(op_2412)) {

			state = ADDCUSTFIELDCANCEL;

		}
		

		String op_2413 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_2413.x"));

		if (AmtCommonUtils.isResourceDefined(op_2413)) {

			state = ADDCUSTFIELDEXTCONTINUE;

		}

		String op_2414 = AmtCommonUtils.getTrimStr((String) etsIssObjKey.getParams().get("op_2414.x"));

		if (AmtCommonUtils.isResourceDefined(op_2414)) {

			state = ADDCUSTFIELDSUBMIT;
	...
	public int processRequest(EtsIssObjectKey etsIssObjKey) {
		...
		EtsCustFieldInfoModel custFieldModel = getCustFieldInfoDets(etsIssObjKey);
			...
			curstate = getAddCustFieldSubActionState(etsIssObjKey);
			//get next state
	...
	String getForwardMap(int processrequest) {
		...
		switch (processrequest) {
			...
			case ADDCUSTFIELD1STPAGE :

				forward = "addCustField1stPage";

				break;

			case ADDCUSTFIELDCONTINUE :

				forward = "addCustField2ndPage";

				break;

			case ADDCUSTFIELDEXTCONTINUE :

				forward = "addCustField3rdPage";

				break;
				
												
			case ADDCUSTFIELDCANCEL :

				forward = "addCustField1stPage";

				break;
				
			case ADDCUSTFIELDSUBMIT :

				forward = "addCustField4thPage";
	...
	public EtsCustFieldInfoModel getCustFieldInfoDets(EtsIssObjectKey etsIssObjKey) throws SQLException, Exception {
		...
		int state = getAddCustFieldSubActionState(etsIssObjKey);
		
		Global.println("current state in getCustFieldInfoDets()===" + state);
		EtsIssAddCustFieldBdlg addCustFieldDataPrep = new EtsIssAddCustFieldBdlg(etsIssObjKey, state); 
		switch (state) {
		
			case ADDCUSTFIELD1STPAGE :
		
				custFieldModel = addCustFieldDataPrep.getFirstPageDets();
		
				break;
		
			case ADDCUSTFIELDCONTINUE :
		
				custFieldModel = addCustFieldDataPrep.getContAddCustFieldDetails();
		
				break;
				
			case ADDCUSTFIELDEXTCONTINUE :
			
				custFieldModel = addCustFieldDataPrep.getExtContAddCustFieldDetails();
		
				break;
						
			case ADDCUSTFIELDSUBMIT :

				custFieldModel = addCustFieldDataPrep.getSubmitAddCustFieldDetails();
