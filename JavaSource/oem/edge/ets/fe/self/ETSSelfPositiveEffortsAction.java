 ...
 */
public class ETSSelfPositiveEffortsAction extends Action {
	/**
	 *
	 */
	...
	public ETSSelfPositiveEffortsAction() {
		super();
	}
		...
		try {
			...
			if (sOp.equals("")) {
				...
				if (validateAtttributesComplete(self,es)) {
					...
					boolean created = ETSSelfDAO.completeMemberSection(con,sSelfId,sProjectId,ETSSelfConstants.SECTION_POSITIVE_EFFORTS,es.gIR_USERN);
	...
	private boolean validateAtttributesComplete(ETSSelfAssessment self, EdgeAccessCntrl es) throws Exception {
		...
		try {
			...
			for (int i = 0; i < listexp.size(); i++) {
				...
				if (exp.getSectionId() == ETSSelfConstants.SECTION_POSITIVE_EFFORTS && exp.getMemberId().equalsIgnoreCase(es.gIR_USERN)) {
