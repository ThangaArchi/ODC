 ...
 */
public class ETSAddPositiveEffortAction extends Action {
	/**
	 *
	 */
	...
	public ETSAddPositiveEffortAction() {
		super();
	}
		...
		try {
			...
			if (sOp.equalsIgnoreCase("edit")) {
				...
				if (key.getOperation().trim().equalsIgnoreCase("save")) {
					...
					key.setExpectId("0");
					key.setSectionId(String.valueOf(ETSSelfConstants.SECTION_POSITIVE_EFFORTS));
					...
					for (int i = 0; i < listexp.size(); i++) {
						...
						//if (exp.getSectionId() == ETSSelfConstants.SECTION_POSITIVE_EFFORTS && exp.getSequenceNo() == Integer.parseInt(sSeqNo) && exp.getProjectId().equalsIgnoreCase(sProjectId) && exp.getSelfId().equalsIgnoreCase(sSelfId) && exp.getMemberId().equalsIgnoreCase(es.gIR_USERN)) {
						if (exp.getSectionId() == ETSSelfConstants.SECTION_POSITIVE_EFFORTS && exp.getSequenceNo() == Integer.parseInt(sSeqNo) && exp.getProjectId().equalsIgnoreCase(sProjectId) && exp.getSelfId().equalsIgnoreCase(sSelfId)) {
