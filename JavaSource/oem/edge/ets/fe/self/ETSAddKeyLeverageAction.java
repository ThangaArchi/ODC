 ...
 */
public class ETSAddKeyLeverageAction extends Action {
	/**
	 *
	 */
	...
	public ETSAddKeyLeverageAction() {
		super();
	}
		...
		try {
			...
			if (sOp.equalsIgnoreCase("edit")) {
				...
				if (key.getOperation().trim().equalsIgnoreCase("save")) {
					...
					key.setExpectId("0");
					key.setSectionId(String.valueOf(ETSSelfConstants.SECTION_KEY_LEVERAGES));
					key.setSubSectionId("1");
					key.setRating("0");
					...
					for (int i = 0; i < listexp.size(); i++) {
						...
						System.out.println("section " + exp.getSectionId());
						System.out.println("section " + exp.getSectionId());

						//if (exp.getSectionId() == ETSSelfConstants.SECTION_KEY_LEVERAGES && exp.getSequenceNo() == Integer.parseInt(sSeqNo) && exp.getProjectId().equalsIgnoreCase(sProjectId) && exp.getSelfId().equalsIgnoreCase(sSelfId) && exp.getMemberId().equalsIgnoreCase(es.gIR_USERN)) {
						if (exp.getSectionId() == ETSSelfConstants.SECTION_KEY_LEVERAGES && exp.getSequenceNo() == Integer.parseInt(sSeqNo) && exp.getProjectId().equalsIgnoreCase(sProjectId) && exp.getSelfId().equalsIgnoreCase(sSelfId)) {
