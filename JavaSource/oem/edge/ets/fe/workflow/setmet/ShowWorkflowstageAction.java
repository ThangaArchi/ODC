 ...
 */
public class ShowWorkflowstageAction extends WorkflowAction {
	...
	public ActionForward executeWorkflow(ActionMapping mapping,
		...
		if(isSuperAdmin(request) || isWspaceOwner(request) ||isWorkflowAdmin(request) || isWspaceMgr(request))
			canAccess = true;
		System.out.println("View canAccess is:" + canAccess + ":");
		...
		sForm.setProjectID(sForm.getProj());
		sForm.setWorkflowID(workflowID);
	...
	private static Log logger	=		WorkflowLogger.getLogger(ShowWorkflowstageAction.class);
}
