 ...
 */
public class CancelWorkflowstageAction extends WorkflowAction {
	...
	public ActionForward executeWorkflow(ActionMapping mapping,
		...
		System.out.println("Cancel action is:" + sForm.getAction() + ":");
		
		boolean canAccess = false;
		if(isSuperAdmin(request) || isWspaceOwner(request) ||isWorkflowAdmin(request) || isWspaceMgr(request))
			canAccess = true;
		System.out.println("Cancel canAccess is:" + canAccess + ":");
		if(canAccess)
        ...
        try{
			...
			smdao.cancelWorkflow(projectID, workflowID, loggedUser);
	...
	private static Log logger	=		WorkflowLogger.getLogger(CancelWorkflowstageAction.class);
}
