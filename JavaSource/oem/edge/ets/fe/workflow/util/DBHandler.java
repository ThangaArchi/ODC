...
public class DBHandler {
	...
	public static Hashtable getHVQueryResults(Connection conn, String qry,
			int vars) throws SQLException {
	...
	public static Vector getValues(Connection conn, String qry)
			throws SQLException {
		return getValues(conn, qry, true);
	}

	public static Vector getValues(Connection conn, String qry, boolean print)
			throws SQLException {
		Vector v = new Vector();
		Statement stmtx = null;
		ResultSet rsx = null;
	...
	public static Hashtable getHQueryResult(Connection conn, String qry)
			throws SQLException {
		Hashtable hs = new Hashtable();
		Statement stmtx = null;
		ResultSet rsx = null;
	...
	public static Vector getVQueryResult(Connection conn, String qry, int vars)
			throws SQLException {
		Vector v = new Vector();
		Statement stmtx = null;
		ResultSet rsx = null;
	...
	public static String getValue(Connection conn, String qry)
			throws SQLException {
		return getValue(conn, qry, true);
	}

	public static String getValue(Connection conn, String qry, boolean print)
			throws SQLException {
		String ret = "";
		Statement stmtx = null;
		ResultSet rsx = null;
	...
	static public int fireUpdate(Connection con, String qry)
			throws SQLException {
		return fireUpdate(con, qry, true);
	}

	static public int fireUpdate(Connection con, String qry, boolean print)
			throws SQLException {
		if (print)
			System.out.println(qry);
		Statement pstmt = null;
	...
	static public int countQry(Connection con, String qry, boolean print)
	...
	public static String getClobValue(Connection conn, String qry, String dir)
			throws SQLException {
		String ret = "", s = "";
		Statement stmtx = null;
		ResultSet rsx = null;
			...
			if (rsx.next()) {
				...
				if (clob != null) {
					...
					if (clobLen < Integer.MAX_VALUE) {
						s = clob
								.getSubString(1, (new Long(clobLen)).intValue());
					}
				}
	...
	public static void setClobValue(Connection conn, String qry, String s)
			throws SQLException {
	...
	public static void saveRecord(Connection conn, String tableName,
			ArrayList values) throws SQLException, Exception {
		Statement stmt = null;
		ResultSet rset = null;
		int iNoofColumns = 0;
		...
		try {
			...
			for (int i = 1; i <= iNoofColumns; i++) {
				...
				sColumnValue = (sColumnValue == null) ? "" : sColumnValue
						.trim();
				coltype = rsmeta.getColumnType(i);
				quoteRequired = !(coltype == Types.FLOAT
						|| coltype == Types.INTEGER || coltype == Types.DECIMAL
						|| coltype == Types.DOUBLE || coltype == Types.NUMERIC
						|| coltype == Types.REAL || coltype == Types.SMALLINT);
				isNull = rsmeta.isNullable(i);
				if ((isNull == ResultSetMetaData.columnNullable)
						&& (coltype == Types.TIMESTAMP || coltype == Types.DATE
								|| coltype == Types.CLOB || coltype == Types.BLOB))
					sColumnValue = "null";
				else if (isNull == ResultSetMetaData.columnNullable
						&& !quoteRequired && sColumnValue.equals(""))
					sColumnValue = "null";
				...
				if (!pk.contains(sColumnName))
					...
					sWhereQry
							.append(sColumnName + "=" + sColumnValue + " and ");
				if (i == 1) {
			...
			if (sColumnUpdateQry.length() > 1)
				updateValues = sColumnUpdateQry.substring(0, sColumnUpdateQry
						.length() - 1);
			if (sWhereQry.length() > 4)
				whereValues = sWhereQry.substring(0, sWhereQry.length() - 4);
			boolean found = countQry(conn, "select * from  " + tableName
					+ " where " + whereValues + " with ur", true) > 0;
			String query = null;
			if (found)
				query = "update " + tableName + " set " + updateValues
						+ " where " + whereValues;
			else
				query = "insert into " + tableName + " ("
						+ sColumnNameInQry.toString() + ") values ("
						+ sColumnValueInQry.toString() + ")";
	...
	public static Hashtable getTableValues(String query, Connection conn)
			throws SQLException, Exception {
		...
		try {
			...
			System.out.println("TEXT: " + tableName + "\t" + schemaName + "\t"
					+ encryptedFields);
			if (rset.next()) {
	...
	public static ArrayList getPrimaryKeys(Connection conn, String tablename)
			throws SQLException {
		...
		rsetdb = dbmeta.getPrimaryKeys(null, tablename.substring(0, idx)
				.toUpperCase(), tablename.substring(idx + 1).toUpperCase());
		while (rsetdb.next()) {
	...
	public static void copySchema(Connection conn, String source, String dest,
			Vector tablelist) throws SQLException {
		conn.commit();
		...
		try {
			for (int i = 0; i < tablelist.size(); i++) {
				...
				fireUpdate(conn, "insert into " + dest + "." + table
