 ...
 */
public class ClientNameFromID implements IDToName {

	private static Log logger = WorkflowLogger.getLogger(ClientNameFromID.class);
	...
	public String convert(String ID, DBAccess db) throws Exception{
		String result = "";
		String q = "select concat(concat(fname,' '), lname) from ets.wf_client where client_id='"+ID+"' with ur";
		db.prepareDirectQuery(q);
		if(db.execute()==1)result=db.getString(0,0);
		return result;
	}
