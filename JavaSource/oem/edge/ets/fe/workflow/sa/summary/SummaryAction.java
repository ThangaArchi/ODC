 ...
 */
public class SummaryAction extends WorkflowAction{
	...
	public ActionForward executeWorkflow(ActionMapping mapping, WorkflowForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		{
			String fileName = msaName.replace(' ','_')+"-summary.pdf";
			

			HashMap m = new HashMap();
			
			m.put("repTitle",mw(nameAndTitle[1]));
			...
			System.out.println("Image src = "+(String)m.get("status"));
			...
			if("pdf".equals(mode))
			{
				...
				pdf2.execute(m,PDFReportMaker.TYPE_MSA_OVERALL,request,response,true);
			}
			else
			{
				InputStream is = null;
				pdf2.execute(m,PDFReportMaker.TYPE_MSA_OVERALL,request,response,false);
				...
				docUtils.addDoc(projectID,wf_id,nameAndTitle[1],fileName, size,is,loggedUser);
		...
		{
			request.setAttribute("repTitle",nameAndTitle[1]);
