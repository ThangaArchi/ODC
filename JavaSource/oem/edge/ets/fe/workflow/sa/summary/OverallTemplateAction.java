...
import oem.edge.ets.fe.workflow.setmet.summary.setmet.*;
/**
 * Class       : SummaryAction
 * Package     : oem.edge.ets.fe.workflow.sa.summary
 * Description : 
 ...
 */
public class OverallTemplateAction extends WorkflowAction{
	...
	public ActionForward executeWorkflow(ActionMapping mapping, WorkflowForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		IdentifyDetails identify = null;
		PrepareDetails prepare = null;
		DBAccess db = null;
		try{
			db = new DBAccess();
			identify = new IdentifyDetails(db,projectID, wf_id);
			prepare = new PrepareDetails(db,projectID, wf_id);
		...
		{
			String fileName = msaName.replace(' ','_')+"-overallTemplate.pdf";
			

			HashMap m = new HashMap();
			
			m.put("repTitle",mw("Overall Template"));
			...
			m.put("status",Global.getUrl("ets/ETSImageServlet.wss?proj=workflow1&mod=100"+c.getStatusBit()));
			...
			m.put("TclientAttendees",mw(c.getClientAttendees()));
			m.put("currentMonth",mw(bl.getPrev0()+" "+bl.getCurrentYear()));
			m.put("msaName",mw(nameAndTitle[0]));
			m.put("TmonthYear",identify.getQuarterNum()+"/"+identify.getYearYY());
			m.put("TclientName",c.getClient());
			m.put("TmeetingDate", identify.getSetmetDate());
			m.put("TibmAttendees",identify.getIBMAttendeesCommaSep());
			
			ArrayList prevs = new ArrayList();
			for(int i=0; i<prepare.getIssueID().size(); i++)
			{
				ArrayList roe = new ArrayList();
				roe.add(prepare.getIssueID().get(i));
				roe.add(prepare.getIssueTitle().get(i));
				prevs.add(roe);
			}
			m.put("TprevIssues",prevs);
			...
			if("pdf".equals(mode))
			{
				...
				pdf2.execute(m,PDFReportMaker.TYPE_MSA_OVERALL_TEMPLATE,request,response,true);
			}
			else
			{
				InputStream is = null;
				pdf2.execute(m,PDFReportMaker.TYPE_MSA_OVERALL_TEMPLATE,request,response,false);
				...
				docUtils.addDoc(projectID,wf_id,"Overall Template",fileName, size,is,loggedUser);
		...
		{
			request.setAttribute("repTitle","Overall Template");
