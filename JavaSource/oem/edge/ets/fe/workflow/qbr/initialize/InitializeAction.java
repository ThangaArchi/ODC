...
public class InitializeAction extends WorkflowAction {
	...
	public ActionForward executeWorkflow(ActionMapping mapping,
		...
		if(vo.getMode().equals("newqbr"))
		{
			request.setAttribute("WF_TYPE_TEXT","QBR");
			request.setAttribute("WF_TYPE","QBR");
	...
	private boolean gatekeeper(ActionMapping mapping, WorkflowForm form, HttpServletRequest request, HttpServletResponse response) {
		...
		String mode = MiscUtils.getPA(request,"mode");
		if(mode==null || !(mode.equals("newqbr") || mode.equals("editqbr")))
		{
			request.setAttribute("WFerr", "Bad mode");
			return false;
		}
		if(mode.equals("editqbr") && workflowID==null && vo.getWorkflowID()==null)
		{
		...
		if(mode.equals("editqbr"))
		{
			//TODO: Checks for whether this is a valid QBR workflow & entitlement checks
		}
		if(mode.equals("newqbr"))
		{
			//TODO: Checks for whether this is a valid QBR workflow & entitlement checks
			if(!MiscUtils.canCreateQBR(projectID))
			{
				request.setAttribute("WFerr", "There can be only one QBR open at a time in a project");
				return false;
			}
		}
		if(company==null)
	...
	private boolean inputValidation(InitializeVO vo, HttpServletRequest request)
	{
		...
		if(vo.getQbrQuarter()==null || vo.getQbrQuarter().length==0)
			errs.add("Please select a quarter for the QBR");
		if(vo.getQbrYear()==null || vo.getQbrYear().length==0)
			errs.add("Please select a year for the QBR");
		...
		if(vo.getMeetingLocation()!=null)
			errs.addAll(CharUtils.getStringErrors(vo.getMeetingLocation(),30,false,"Meeting location"));
		String mode = MiscUtils.getPA(request,"mode");
		if("editqbr".equals(mode) && !(request.getAttribute("CURRENT_STAGE")==null
		...
		boolean proceedParsingBiweekly = true;
		...
		if(proceedParsingBiweekly && vo.getBiweeklyFlag()[0].equals("Y"))
		{
			...
			{	
				...
				if("editqbr".equals(mode) && currentBiweeklyFlag.equals("Y"))
				{
